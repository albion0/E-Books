...
import { Drawer, Form, Button, Row, DatePicker, Spin, Upload } from "antd";
import {
  LoadingOutlined,
  PlusOutlined,
  MinusOutlined,
...
export const EditBook = ({ visible, recallAllBooks, handleModal, bookId }) => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const getOneResponse = useSelector(({ books }) => books.getOne);
  const updatedBookResponse = useSelector(({ books }) => books.updateOne);
  ...
  useEffect(() => {
    if (getOneResponse && getOneResponse.loading) {
      ...
      const { book } = getOneResponse.data;

      const authorIds = book.authors.map((cat) => cat._id);
      const genreIds = book.genres.map((skill) => skill._id);

      setValue("title", book.title);
      setValue("credits", book.credits);
      setValue("authors", authorIds);
      setValue("genres", genreIds);
      setValue("content", book.content);
      setAuthorChips(book.authors);
      setGenreChips(book.genres);
      setPreviousPhoto(book.bookPhoto);
  ...
  useEffect(async () => {
    if (updatedBookResponse && updatedBookResponse.success) {
      const { book } = updatedBookResponse.data;
        ...
        await dispatch(
          uploadPhotoBook(
            { bookId: book._id, formData },
            {
              ...
              history,
              pathname: null,
              onSuccessMessage: "Book updated successfully!",
              onFailMessage: "Failed to update book!",
            }
          )
        );
      }
  ...
  const onSubmit = async (data) => {
    ...
    const payload = {
      ...
      authors: data.authors.map((aut) => (aut._id ? aut._id : aut)),
      genres: data.genres.map((gen) => (gen._id ? gen._id : gen)),
      credits: data.credits,
    };

    await dispatch(
      updateOneBook(payload, {
        showToast: uploadedFile ? false : true,
        toastNotification,
        history,
        pathname: null,
        onSuccessMessage: "Book updated successfully!",
                  ...
                  authorsResponse.data.authors.docs.map((author) => {
                    return (
                      <option
                        ...
                        selected={selectedAuthor === author._id}
                      >
                        {author.name}
                      </option>
                    );
          ...
          <Row style={{ marginTop: "5px" }}>
            <div style={{ width: "100%" }}>
              ...
              <input
                ...
                className="form-control"
                id="basic-url"
                defaultValue={getValues("credits") || ""}
